...
import { osaAPI, setAuthToken } from '../../lib/api';
...
export default function OSAScreen() {
      ...
      [
        {
          ...
          onPress: async () => {
            try {
              ...
              if (status !== 'granted') {
                Alert.alert('Permission Denied', 'Camera permission is required to take photos. Please enable camera access in your device settings.');
                return;
              }
        ...
        },
        {
          ...
          onPress: async () => {
            try {
              ...
              if (status !== 'granted') {
                Alert.alert('Permission Denied', 'Photo library permission is required. Please enable photo access in your device settings.');
                return;
              }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      formData.append('availability', JSON.stringify(products)); // Backend expects "availability"
      
      // Handle photo upload - React Native specific format
      const photoFile = {
        uri: Platform.OS === 'ios' ? photo.uri.replace('file://', '') : photo.uri,
        type: photo.type || 'image/jpeg',
        name: photo.fileName || `osa-${Date.now()}.jpg`,
      };
      formData.append('photo', photoFile);

      console.log('Submitting OSA with:', {
        store_id: selectedStore.id,
        products: products.length,
        hasPhoto: !!photo.uri,
      });

      const response = await osaAPI.create(formData);
      console.log('OSA submission response:', response.data);
      ...
      let errorMessage = 'Failed to submit OSA record.';
    ...
    <View style={styles.wrapper}>
      ...
      <ScrollView style={styles.container}>
        ...
        <View style={styles.section}>
          ...
          {/* Map Display (Native platforms only) */}
